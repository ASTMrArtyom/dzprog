...
def plot_results(antenna, theta_deg, D_theta, D_theta_db, cst_data=None):
    ...
    if cst_data is not None:
        ...
        theta_cst_full = np.concatenate([theta_cst, theta_cst[1:-1] + 180])
        D_theta_cst_full = np.concatenate([D_theta_cst, D_theta_cst[1:-1]])
        ax1.plot(theta_cst_full, D_theta_cst_full, 'ro', markersize=4, alpha=0.7, label='CST данные')
        ax1.legend()
    
    ax1.set_xlabel('θ, градусы')
    ax1.set_ylabel('D(θ)')
    ax1.set_title('Диаграмма направленности')
    ax1.grid(True)
    ax1.set_xlim(0, 360)
    ...
    if cst_data is not None:
        theta_cst, D_theta_cst = cst_data
        D_theta_cst_db = 10 * np.log10(np.maximum(D_theta_cst, 1e-10))
        theta_cst_full = np.concatenate([theta_cst, theta_cst[1:-1] + 180])
        D_theta_cst_db_full = np.concatenate([D_theta_cst_db, D_theta_cst_db[1:-1]])
        ax2.plot(theta_cst_full, D_theta_cst_db_full, 'ro', markersize=4, alpha=0.7, label='CST данные')
        ax2.legend()
    
    ax2.set_xlabel('θ, градусы')
    ax2.set_ylabel('D(θ), дБ')
    ax2.set_title('Диаграмма направленности')
    ax2.grid(True)
    ax2.set_xlim(0, 360)
    ...
    if cst_data is not None:
        ...
        theta_cst_rad_full = np.concatenate([theta_cst_rad, theta_cst_rad[1:-1] + np.pi])
        D_theta_cst_full = np.concatenate([D_theta_cst, D_theta_cst[1:-1]])
        ax3.plot(theta_cst_rad_full, D_theta_cst_full, 'ro', markersize=4, alpha=0.7, label='CST данные')
        ax3.legend(loc='upper right', bbox_to_anchor=(1.5, 1.0))
    
    ax3.set_theta_zero_location('N')
    ax3.set_theta_direction(-1)
    ax3.set_title('Диаграмма направленности')
    ...
    if cst_data is not None:
        theta_cst, D_theta_cst = cst_data
        D_theta_cst_db = 10 * np.log10(np.maximum(D_theta_cst, 1e-10))
        D_theta_cst_db_normalized = D_theta_cst_db - np.min(D_theta_cst_db)
        if np.max(D_theta_cst_db_normalized) > 0:
            D_theta_cst_db_normalized = D_theta_cst_db_normalized / np.max(D_theta_cst_db_normalized)
        
        theta_cst_rad = np.radians(theta_cst)
        theta_cst_rad_full = np.concatenate([theta_cst_rad, theta_cst_rad[1:-1] + np.pi])
        D_theta_cst_db_normalized_full = np.concatenate([D_theta_cst_db_normalized, D_theta_cst_db_normalized[1:-1]])
        
        ax4.plot(theta_cst_rad_full, D_theta_cst_db_normalized_full, 'ro', markersize=4, alpha=0.7, label='CST данные')
        ax4.legend(loc='upper right', bbox_to_anchor=(1.5, 1.0))
    
    ax4.set_theta_zero_location('N')
    ax4.set_theta_direction(-1)
    ax4.set_title('Диаграмма направленности')
    
    plt.tight_layout()
    fig.savefig('dipole_pattern_combined_with_cst_original.png', dpi=300, bbox_inches='tight')
...
def main():
    ...
    for i, theta in enumerate(theta_rad):
        if theta <= np.pi:
            ...
            D_theta[i] = antenna.directivity(2*np.pi - theta)
...
def save_data(theta_deg, D_theta, D_theta_db):
    with open('dipole_pattern_data_original.csv', 'w') as f:
        f.write("Theta(deg),D(linear),D(dB)\n")
        for i in range(len(theta_deg)):
            f.write(f"{theta_deg[i]},{D_theta[i]},{D_theta_db[i]}\n")
    
    print("\nДанные сохранены в файл 'dipole_pattern_data_original.csv'")
